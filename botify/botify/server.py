...
user_sessions_redis = Redis(app, config_prefix="REDIS_USER_SESSIONS")
...
catalog.upload_tracks(tracks_redis.connection, my_tracks_redis.connection)
    ...
    def post(self, user: int):
        ...
        if treatment == Treatment.T1:
            recommender = MyRecommender(
                app,
                my_tracks_redis.connection,
                artists_redis.connection,
                user_sessions_redis.connection,
                catalog,
                log_predict_score=False,
...
class LastTrack(Resource):
    def post(self, user: int):
        start = time.time()
        args = parser.parse_args()
        user_sessions_redis.connection.set(
            user,
            catalog.to_bytes({'all_tracks': [], 'good_tracks': []})
        )
        data_logger.log(
            "last",
            Datum(
